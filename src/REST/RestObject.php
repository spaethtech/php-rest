    ...
    public function toJSON(string $method = "", bool $filter = true, int $options = 0): string
    {
        ...
        foreach($properties as $property)
        {
            ...
            if(!array_key_exists("types", $info))
                throw new Exception("[rspaeth\Annotations\AnnotationReader] Unable to successfully parse the DocBlock for '$name', ".
                    "missing @var type!");
                ...
                $method === "")
            {
                ...
                if($type !== null && !in_array($type, $types))
                {
                    ...
                    if(is_subclass_of($type, $base, true))
                    {
                        ...
                        foreach($children as $child)
                        {
                            ...
                            else
                            {
                                ...
                                throw new Exception("[rspaeth\REST\RestObject] An array was found for which a Lookup class should be ".
                    ...
                    else
                    {
                        ...
                        // We should NEVER reach this block, in theory!
                        throw new Exception("[rspaeth\REST\RestObject] An object was found that does not extend from Lookup: $type");
                    }
                }
                else
                {
        ...
        foreach($properties as $property)
        {
            ...
            if(array_key_exists(ucfirst($method)."Required", $params) || array_key_exists($method."-required", $params))
            {
                ...
                if ($conditional === "")
                {
                    ...
                    {
                        throw new Exception("[rspaeth\Annotations\AnnotationReader] An annotation of '@{$method}Required' needs to either have a ".
