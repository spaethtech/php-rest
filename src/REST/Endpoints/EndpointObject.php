...
use SpaethTech\Common\{Arrays, Patterns, Strings};

use Psr\Http\Message\ResponseInterface;
use SpaethTech\REST\{RestObject, RestClient};
...
abstract class EndpointObject extends RestObject
{
    ...
    public function insert(): self
    {
        ...
        {
            throw new \Exception("[SpaethTech\REST\Endpoints\EndpointObject] Annotations for the '".get_class($this)."' class require valid values be set ".
    ...
    public static function post(EndpointObject $data, array $params = [], ResponseInterface &$response = null): EndpointObject
    {
        ...
        if($endpoint === "")
            throw new \Exception("[SpaethTech\REST\Endpoints\EndpointObject] An annotation like '@Endpoint { \"post\": \"/examples\" }' on the '$class' ".
        ...
        if(array_key_exists("code", $body))
        {
            switch($body["code"])
            {
                case 400: throw new \Exception("[SpaethTech\REST\Endpoints\EndpointObject] {$body["message"]}");
                case 401: throw new \Exception("[SpaethTech\REST\Endpoints\EndpointObject] The REST Client was not authorized to make this request!");
                case 403: throw new \Exception("[SpaethTech\REST\Endpoints\EndpointObject] The provided App Key does not have sufficient privileges!");
                case 404: throw new \Exception("[SpaethTech\REST\Endpoints\EndpointObject] EndpointObject '$endpoint' was not found for class '$class'!");
                case 422: throw new \Exception("[SpaethTech\REST\Endpoints\EndpointObject] Data for endpoint '$endpoint' was improperly formatted!\n".
                    $body["message"]."\n".
    ...
    public static function get(string $override = "", array $params = [], array $query = []): Collection
    {
        ...
        if($override === "")
        {
            ...
            if (!array_key_exists("get", $endpoints) || $endpoints["get"] === "")
                throw new \Exception("[SpaethTech\REST\Endpoints\Endpoint] An '@Endpoint { \"get\": \"/examples\" }' annotation on the class must " .
        ...
        if(array_key_exists("code", $response))
        {
            ...
            {
                case 400: throw new \Exception("[SpaethTech\REST\Endpoints\EndpointObject] {$response["message"]}");
                case 401: throw new \Exception("[SpaethTech\REST\Endpoints\EndpointObject] The REST Client was not authorized to make this request!");
                case 403: throw new \Exception("[SpaethTech\REST\Endpoints\EndpointObject] The provided App Key does not have sufficient privileges!");
                case 404: throw new \Exception("[SpaethTech\REST\Endpoints\EndpointObject] EndpointObject '$endpoint' was not found for class '$class'!");

                case 500: throw new \Exception("[SpaethTech\REST\Endpoints\EndpointObject] EndpointObject '$endpoint' returned: '{$response["message"]}'!");
    ...
    public static function getById(?int $id): ?EndpointObject
    {
        ...
        if(!array_key_exists("getById", $endpoints))
            throw new \Exception("[SpaethTech\REST\Endpoints\EndpointObject] An '@Endpoint { \"getById\": \"/examples/:id\" }' annotation on ".
    ...
    public static function patch(?EndpointObject $data, array $params = [], string $suffix = ""): ?EndpointObject
    {
        ...
        if($endpoint === null || $endpoint === [])
            throw new \Exception("[SpaethTech\REST\Endpoints\EndpointObject] An annotation like '@Endpoint { \"patch\": \"/examples/:id\" }' on the ".
    ...
    public static function delete(?EndpointObject $data, array $params = [], string $suffix = ""): bool
    {
        ...
        if($endpoint === null || $endpoint === [])
            throw new \Exception("[SpaethTech\REST\Endpoints\EndpointObject] An annotation like '@Endpoint { \"delete\": \"/examples/:id\" }' on the ".
                "'$class' class must be declared in order to resolve this endpoint'");
